...
int rdt3_send(int sockfd, struct sockaddr_in servaddr, char ack_num, char *buffer, unsigned len) {
	...
	while (waiting) {
		...
		int n = recvfrom(sockfd, (char *)recv_buf, MAXMSG,
				MSG_WAITALL, ( struct sockaddr *) &recv_addr, &recv_len); // Receive the response
		if (n < 0) {
			continue;
		}
		...
		/* If receive ACK signal, stop receiving. */
		if (strncmp(recv_buf + parse_idx, ACK, strlen(ACK)) == 0) {
			waiting = 0;
		}
...
int p2p_client(unsigned int ip, unsigned short port, char *file_name) {
    ...
    char file_size_str[MAXMSG] = {0};
    if (recv(sock, file_size_str, MAXMSG, 0) < 0) {
    ...
    // Convert the file size from string to long
    long file_size = atol(file_size_str);
    if (file_size == 0) {
        fprintf(stderr, "Invalid file size received or file does not exist.\n");
        fclose(fp);
        close(sock);
        return -1;
    }
    ...
    // Initialize a counter to track the number of bytes received
    long total_bytes_received = 0;

    // Loop to receive the file contents in chunks
